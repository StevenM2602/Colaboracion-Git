<?php require("layout/header.php"); ?>
<h1>LINEAS DE FACTURAS</h1>
<br />
<table border="1">
    <tr><th>Nombre: </th><td><?php echo $factura->nombre; ?></td></tr>
    <tr><th>Número: </th><td><?php echo $factura->numero; ?></td></tr>
    <tr><th>Fecha: </th><td><?php echo $factura->fecha; ?></td></tr>
</table>

<br>
<table class="table table-striped table-hover" id="tabla">
    <thead>
        <tr class="text-center">
            <th>Id</th>
            <th>Factura N°</th>
            <th>Referencia</th>
            <th>Descripción</th>
            <th>Cantidad</th>
            <th>Precio</th>
            <th>Iva</th>
            <th>Importe</th>
            <th></th>
        </tr>
    </thead>
    <tbody>
        <?php
        if ($lineas->filas) :
            foreach ($lineas->filas as $fila) :
        ?>
                <tr>
                    <td style="text-align: center; width: 5%;"><?php echo $fila->id; ?></td>
                    <td style="text-align: center;"><?php echo $fila->factura_id; ?></td>
                    <td style="text-align: center;"><?php echo $fila->referencia; ?></td>
                    <td style="text-align: center;"><?php echo $fila->descripcion; ?></td>
                    <td style="text-align: center;"><?php echo $fila->cantidad; ?></td>
                    <td style="text-align: center;"><?php echo $fila->precio; ?></td>
                    <td style="text-align: center;"><?php echo $fila->iva; ?></td>
                    <td style="text-align: center;"><?php echo $fila->importe; ?></td>
                    
                    <td style="text-align: right; width: 50%;">
                        <a href="index.php?c=lineas_factura&m=editar&id=<?php echo $fila->id; ?>">
                            <button type="button" class="btn btn-success">Editar</button></a>
                        <a href="index.php?c=lineas_factura&m=borrar&id=<?php echo $fila->id; ?>">
                            <button type="button" class="btn btn-danger borrar"
                                onclick="return confirm('¿Estás seguro de borrar la línea<?php
                                echo $fila->id; ?>?');">Borrar</button></a>
                    </td>
                </tr>
        <?php
            endforeach;
        endif;
        ?>
    </tbody>
    <tfoot>
        <tr>
            <td colspan="4">
                <a href="index.php?c=lineas_factura&m=nuevo&factura_id=<?php echo $factura->id;?>">
                    <button type="button" class="btn btn-primary">Nuevo</button>
                </a>
                <a href="index.php?c=facturas&m=atras">
                    <button type="button" class="btn btn-danger">Atras</button>
                </a>
            </td>
        </tr>
    </tfoot>
</table>
<?php require("layout/footer.php"); ?>